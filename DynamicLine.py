...
import os
class TimeLine(DynamicLine):
    def __init__(self,refreshTime):
        DynamicLine.__init__(self,refreshTime)
        self._lastTime = None
        self._lastLabel = None

    def updateLabel(self,label=None):
        self.overwrite(label)
        sleep(self._refreshTime)

    def updateTimer(self,timer,label=None):
        ''' Update the timer '''
        time = timer.check()
        self.overwrite(time)
        sleep(self._refrehTime)
